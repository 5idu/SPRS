...
import FPPoDetailBody from '../components/FPPoDetailBody';
...
const poDetailData = (UserID,SoNum,State) => {
    return (dispatch,getState) => {	
		...
		let url = "http://jisapp.jhtgroup.com/FinishedProducts/Home/PoSearch?UserID="+ UserID +"&SoNum="+ SoNum +"&State="+ State;
...
const FPPoDetailForm = connect(
  mapStateToProps,
  mapDispatchToProps
)(FPPoDetailBody);

export default class FPPoDetail extends React.Component {
	render(){
		return (
		  <div>
		  	<FPHeader {...this.props}/>
		  	<div>
		  		<FPPoDetailForm {...this.props}/>
		  	</div>
		  </div>
		)	
	}
